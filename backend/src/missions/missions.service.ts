...
export class MissionsService implements OnModuleInit {
    ...
    private async ensureUserMissions(userId: string) {
        ...
        const userMissions = await this.userMissionsRepository.find({ where: { userId } });

        const existingMissionIds = userMissions.map(um => um.missionId);
        const missingMissions = missions.filter(m => !existingMissionIds.includes(m.id));
    ...
    async trackProgress(userId: string, type: MissionRequirementType, amount: number = 1) {
        await this.ensureUserMissions(userId);
    ...
    async claimReward(userId: string, userMissionId: number) {
        return this.dataSource.manager.transaction(async (entityManager) => {
            ...
            if (!userMission.isCompleted) throw new BadRequestException('Mission not completed yet');
    ...
    async handleDailyReset() {
        ...
        const dailyMissions = await this.missionsRepository.find({ where: { type: MissionType.DAILY } });
