...
export class MarketService implements OnModuleInit {
  ...
  // Listázza az összes shop itemet (cached)
  async getShopItems(): Promise<Item[]> {
    ...
    // Step 3: Save to cache (TTL: 5 minutes = 300 seconds)
  ...
  async buyItem(
    userId: string,
    itemId: string,
  ): Promise<{ message: string; remainingCash: string }> {
    ...
    return await this.usersRepository.manager.transaction(async (manager) => {
      ...
      if (inventory.isEquipped) {
        throw new BadRequestException(
          'Nem listázhatod, amíg fel van szerelve!',
        );
      }
  ...
  async buyListing(
    buyerId: string,
    listingId: string,
  ): Promise<{ message: string; item: string }> {
    return await this.usersRepository.manager.transaction(async (manager) => {
