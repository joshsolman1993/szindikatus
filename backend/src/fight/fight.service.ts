...
export class FightService {
  ...
  canAttack(
    ...
    if (attacker.nerve < GameBalance.FIGHT_NERVE_COST) {
      return {
        canAttack: false,
        reason: 'Nincs elég bátorságod a támadáshoz!',
      };
    }
  ...
  calculateOutcome(
    ...
    let attackerScore =
      (attackerStats.totalStr + attackerStats.totalSpd) *
      (0.8 + Math.random() * 0.4);
    let defenderScore =
      (defenderStats.totalTol + defenderStats.totalSpd) *
      (0.8 + Math.random() * 0.4);

    // Talent Bonuses
    if (
      attackerStats.learnedTalents &&
      attackerStats.learnedTalents.includes(TalentId.SHARPSHOOTER)
    ) {
      attackerScore *= 1.1;
    }
    if (
      defenderStats.learnedTalents &&
      defenderStats.learnedTalents.includes(TalentId.IRON_SKIN)
    ) {
      defenderScore *= 1.05;
    }
  ...
  async executeFight(
    ...
    return await this.usersRepository.manager.transaction(async (manager) => {
      ...
      const firstUser = await manager.findOne(User, {
        where: { id: firstId },
        lock: { mode: 'pessimistic_write' },
      });
      const secondUser = await manager.findOne(User, {
        where: { id: secondId },
        lock: { mode: 'pessimistic_write' },
      });
      ...
      const attackerStats =
        await this.usersService.calculateCombatStats(attackerId);
      const defenderStats =
        await this.usersService.calculateCombatStats(defenderId);
      ...
      if (attackerWins) {
        ...
        if (attackerStats.equippedItems.length > 0) {
          logs.push(
            `A(z) ${attackerStats.equippedItems.join(', ')} segítségével extra sebzést vittél be!`,
          );
        }
        ...
        moneyStolen = Math.floor(
          defenderCash * GameBalance.FIGHT_CASH_STEAL_PERCENTAGE,
        );
        ...
        this.missionsService.trackProgress(
          attackerId,
          MissionRequirementType.FIGHT_WIN,
          1,
        );
      } else {
        // VERESÉG
        ...
        if (defenderStats.equippedItems.length > 0) {
          logs.push(
            `Az ellenfél a(z) ${defenderStats.equippedItems.join(', ')} segítségével hárított!`,
          );
        }
