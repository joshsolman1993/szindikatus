...
export class PropertiesService implements OnModuleInit {
            ...
            const properties = [
                {
                    ...
                    cost: 2000,
                    incomePerHour: 50,
                    imageUrl: 'https://images.unsplash.com/photo-1605218427306-6354db69e563?w=500&auto=format&fit=crop&q=60',
                },
                {
                    ...
                    cost: 15000,
                    incomePerHour: 500,
                    imageUrl: 'https://images.unsplash.com/photo-1516937941348-c09645f8b434?w=500&auto=format&fit=crop&q=60',
                },
                {
                    name: 'Éjszakai Klub',
                    description: 'Ahol az alvilág krémje mulat. Magas bevétel, magas kockázat.',
                    cost: 150000,
                    incomePerHour: 4000,
                    imageUrl: 'https://images.unsplash.com/photo-1566737236500-c8ac43014a67?w=500&auto=format&fit=crop&q=60',
                },
                {
                    name: 'Fegyvergyár',
                    ...
                    cost: 2000000,
                    incomePerHour: 50000,
                    imageUrl: 'https://images.unsplash.com/photo-1580234550905-408d43d48d01?w=500&auto=format&fit=crop&q=60',
                },
            ];
    ...
    async buyProperty(userId: string, propertyId: number) {
        return this.dataSource.manager.transaction(async (entityManager) => {
            ...
            if (!user || !property) {
                throw new BadRequestException('Felhasználó vagy ingatlan nem található.');
            }
            ...
            if (currentCash < cost) {
                throw new BadRequestException('Nincs elég pénzed az ingatlan megvásárlásához.');
            }
    ...
    async collectIncome(userId: string) {
        return this.dataSource.manager.transaction(async (entityManager) => {
            ...
            return {
                ...
                message: totalIncome > 0 ? `Sikeresen begyűjtöttél $${totalIncome}-t!` : 'Még nem gyűlt össze elég bevétel.'
            };
        });
    }
}
