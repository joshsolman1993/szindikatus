...
export class CrimesService implements OnModuleInit {
  ...
  async commitCrime(userId: string, crimeId: number) {
    ...
    return this.dataSource.manager.transaction(async (entityManager) => {
      ...
      const crime = await entityManager.findOne(Crime, {
        where: { id: crimeId },
      });
      ...
      if (user.energy < crime.energyCost) {
        throw new BadRequestException(
          'Nincs elég energiád a bűntény elkövetéséhez.',
        );
      }
      ...
      if (isSuccess) {
        // Jutalom
        let moneyReward =
          Math.floor(Math.random() * (crime.maxMoney - crime.minMoney + 1)) +
          crime.minMoney;

        // Talent Bonus: Street Wisdom (+5% Money)
        if (
          user.learnedTalents &&
          user.learnedTalents.includes(TalentId.STREET_WISDOM)
        ) {
        ...
        if (Math.random() < dropChance) {
          ...
          if (itemCount > 0) {
            ...
            if (randomItem) {
              const lootedItem = await this.lootService.generateLoot(
                randomItem.id,
                userId,
              );
        ...
        if (district && district.ownerClan) {
          ...
          if (taxAmount > 0) {
            // Add to clan bank
            const clan = await entityManager.findOne(Clan, {
              where: { id: district.ownerClan.id },
            });
            if (clan) {
        ...
        result.message =
          'A bűntény nem sikerült! Menekülnöd kellett és megsérültél.';
      }
