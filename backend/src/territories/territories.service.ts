...
export class TerritoriesService implements OnModuleInit {
    ...
    private async seedDistricts() {
        ...
        if (count === 0) {
            ...
            const districts = [
                {
                    name: 'Kikötő',
                    description: 'A csempészek paradicsoma. Könnyű célpont, de a bevétel is szerény.',
                    defense: 500,
                    maxDefense: 500,
                    taxRate: 0.03,
                ...
                {
                    name: 'Ipari Zóna',
                    description: 'Gyárak és raktárak. A munkások kemények, de a pénz biztos.',
                    defense: 1000,
                    maxDefense: 1000,
                    taxRate: 0.05,
                ...
                },
                {
                    ...
                    description: 'Bankok és tőzsde. Csak a legerősebbek merészkednek ide.',
                    defense: 4000,
                    maxDefense: 4000,
                    taxRate: 0.15,
    ...
    async attackDistrict(userId: string, districtId: number) {
        return this.dataSource.manager.transaction(async (entityManager) => {
            ...
            if (!user.clan) throw new BadRequestException('You must be in a clan to attack territories.');
            ...
            if (action === 'attack') {
                ...
                if (district.defense <= 0) {
                    // Capture!
                    const oldOwnerName = district.ownerClan ? district.ownerClan.name : 'Senki';
                ...
                // Reinforce
                const repairAmount = Math.floor(user.stats.str * 2 + Math.random() * 10 + 10);
                district.defense = Math.min(district.maxDefense, district.defense + repairAmount);
            ...
            return {
                ...
                district: {
                    ...
                    ownerClan: district.ownerClan ? { id: district.ownerClan.id, name: district.ownerClan.name, tag: district.ownerClan.tag } : null,
                },
                user: {
                    nerve: user.nerve,
                    energy: user.energy,
